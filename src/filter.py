...
class Filter():
	...
	def __get_pubmed_features__(self, text):
		...
		#chem_term_count = self.__chemical_counts__(chemical_disease_nlp,text)
	...
	def __chemical_counts__(self,spacy_nlp,content):
		chemical_disease_doc = spacy_nlp(content)
		chemical_ents = [ent.text for ent in chemical_disease_doc.ents if ent.label == 'CHEMICAL']
		return len(chemical_ents)
